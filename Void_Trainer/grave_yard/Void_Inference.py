...
class Void_Inference:
    ...
    def TextGeneration_backup(self):
        
        print(f'{Fore.LIGHTRED_EX}Warning: Multi GPU will be disabled when using backup_mode.{Fore.RESET}')
    ...
    def TextGeneration_chat_backup(self):
    ...
    def inference(self):
        ...
        if self.inference_type == 'TxtGen':
            if self.backup_mode:
                self.TextGeneration_backup()
                
                
        
        if self.inference_type == 'Chat':
            if not self.backup_mode:
                self.TextGeneration_chat()
                
                
        if self.inference_type == 'Chat':
            if self.backup_mode:
                self.TextGeneration_chat_backup()
